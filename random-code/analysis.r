...
# a number of helper functions you can use from within R consoles.
...
# -----------------
#
# Worker consoles run on remote containers and can be networked together
...
# a single master console for easy debugging.
#
# The following command launches two workers and runs some code remotely:

#workers <- launch.workers(n=2, size=1, code="print('Hello from Sense Worker')")

# You can stop workers with `stop.workers()`.  All workers will automatically
# be cleaned up when you stop the console as well (top-right console drop down).
#
# In most cases you'll want to use a higher-level library to distribute
# your analysis.  See the [documentation](http://help.sense.io) to learn more.
#
# There's lots more to Sense.  If you have any problems, email support@sense.io.
