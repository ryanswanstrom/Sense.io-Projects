...
# -----------------
# 
# Worker dashboards run on remote containers and can be networked together
...
# a single master dashboard for easy debugging.
#
# The following command launches two workers and runs some code remotely:

workers <- launch.workers(n=2, code="print('Hello from Sense Worker')")

# You can stop workers with `stop.workers()`.  All workers will automatically
# be cleaned up when you stop the dashboard as well (top-right dashboard drop down). 
#
# In most cases you'll want to use a higher-level library to distribute 
# your analysis.  See the [documentation](http://help.senseplatform.com) to learn more.
#
# There's lots more to Sense.  If you have any problems, email support@senseplatform.com.
